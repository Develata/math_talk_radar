...
mod tests {
    ...
    #[test]
    fn canonical_url_preserves_session_identity_and_nondefault_port() {
        let canonical = |text| super::canonicalize_url(&url::Url::parse(text).unwrap());
        assert_eq!(
            canonical(
                "https://www.example.org:8443/talk/?session=2&UTM_SOURCE=feed&ref=calendar#slides"
            ),
            "https://example.org:8443/talk?ref=calendar&session=2"
        );
        assert_ne!(
            canonical("https://example.org/talk?session=1"),
            canonical("https://example.org/talk?session=2")
        );
        assert_eq!(
            canonical("https://www.example.org:443/?gclid=tracking"),
            "https://example.org/"
        );
    }
